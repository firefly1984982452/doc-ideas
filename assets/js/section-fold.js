(function () {
  ...
  function isFoldablePage() {
    return /^#\/docs\/(?:idea|letter)\//.test(currentRoute());
  }
  ...
  function stateKey(heading, index) {
    return 'doc-ideas:fold:' + currentRoute() + ':' + (heading.id || index);
  }
  ...
  function saveCollapsed(key, collapsed) {
    ...
    catch (error) { /* Folding still works without storage. */ }
  }
  ...
  function setCollapsed(heading, button, nodes, collapsed, options) {
    ...
    button.setAttribute('aria-label', (collapsed ? '展开' : '折叠') + '“' + sectionTitle(heading) + '”');
    button.setAttribute('title', collapsed ? '展开本节' : '折叠本节');
  ...
  function mountHeading(heading, index) {
    ...
    button.addEventListener('click', function (event) {
      ...
      setCollapsed(heading, button, nodes, button.getAttribute('aria-expanded') === 'true', { animate: true, persist: true });
    });
  }
  ...
  function mount() {
    ...
    if (!article) return;
    Array.from(article.children).filter(function (element) { return element.tagName === 'H2'; }).forEach(mountHeading);
